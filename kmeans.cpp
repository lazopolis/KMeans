...
bool evaluateCond(const vector<DataPoint> &old_centroids, const vector<DataPoint> &new_centroids, double error)
{
...
void Cluster::updateCentroid()
{
     ...
     for (int j=0;j<_points.size();++j)
          ...
          for (int i=0;i<_points[j].coords.size();++i)
               {
               new_centroid.coords[i] += _points[j].coords[i] / _points.size();
               }
...
void Kmeans::_initialize()
{
     ...
     for (size_t i=0; i<_number_of_clusters; ++i)
          {
          int centroid_id = rand() % _data_set.size();
